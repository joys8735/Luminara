...
export function AuthModal({
  ...
  return <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#121212] border border-[#1f1f1f] rounded-lg max-w-md w-full p-6 relative animate-fadeIn">
        <button onClick={onClose} disabled={isLoading} className="absolute top-4 right-4 text-[#a0a0a0] hover:text-[#e0e0e0] transition-colors disabled:opacity-50">
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-[#a0a0a0]">
            {mode === 'login' ? 'Sign in to your account to continue' : 'Join Luminara and start investing'}
          </p>
        </div>

        {/* Google & Metamask Login Buttons */}
        <div className="space-y-3 mb-4">
          <button
            onClick={handleGoogleLogin}
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-gradient-to-r from-[#4285F4] to-[#34A853] hover:from-[#3b7ae8] hover:to-[#2e9c57] disabled:opacity-70 border border-[#1f1f1f] rounded-lg text-white font-medium text-xs transition-colors"
          >
            {isLoading ? (
              <>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-[#f6851b] hover:bg-[#e67e22] disabled:opacity-70 border border-[#1f1f1f] rounded-lg text-white font-medium text-xs transition-colors"
          >
            {isLoading ? (
              <>
                <Loader className="w-4 h-4 animate-spin" />
