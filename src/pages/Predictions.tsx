...
export function Predictions() {
  ...
  // Load real sports and news data
  useEffect(() => {
    async function loadSportsData() {
      ...
      try {
        ...
        if (events && events.length > 0) {
          ...
          // Update sportsAsPairs with real data
          console.log('Loaded sports events:', formatted);
        }
      } catch (error) {
    ...
    async function loadNewsData() {
      ...
      try {
        ...
        if (news && news.length > 0) {
          // Update newsAsPairs with real data
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setActiveBets((prev) => {
        ...
        prev.forEach((bet) => {
          if (now >= bet.endAt) {
            // Знаходимо пару в залежності від ID (crypto: 1-9, sports: 101-108, news: 201-208)
            let pair: Pair | undefined;
            let lastPrice: number;
            
            if (bet.id >= 1 && bet.id <= 9) {
              // Crypto
              pair = pairsRef.current.find((p) => p.id === bet.id);
              lastPrice = pair?.price ?? bet.entryPrice;
            } else if (bet.id >= 101 && bet.id <= 108) {
              // Sports - використовуємо випадковий результат з невеликим bias
              pair = sportsAsPairs.find((p) => p.id === bet.id);
              // Для спорту: 55% шанс виграшу якщо ставка співпадає з трендом
              const trendUp = (pair?.change ?? 0) > 0;
              const betMatchesTrend = (bet.dir === 'up' && trendUp) || (bet.dir === 'down' && !trendUp);
              const winChance = betMatchesTrend ? 0.55 : 0.45;
              lastPrice = Math.random() < winChance ? (bet.dir === 'up' ? bet.entryPrice + 0.1 : bet.entryPrice - 0.1) : (bet.dir === 'up' ? bet.entryPrice - 0.1 : bet.entryPrice + 0.1);
            } else if (bet.id >= 201 && bet.id <= 208) {
              // News - використовуємо sentiment для визначення результату
              pair = newsAsPairs.find((p) => p.id === bet.id);
              const sentiment = (pair?.change ?? 0);
              // Якщо sentiment позитивний і ставка BULLISH (up) - більше шансів виграти
              const betMatchesSentiment = (bet.dir === 'up' && sentiment > 0) || (bet.dir === 'down' && sentiment < 0);
              const winChance = betMatchesSentiment ? 0.6 : 0.4;
              lastPrice = Math.random() < winChance ? (bet.dir === 'up' ? bet.entryPrice + 0.1 : bet.entryPrice - 0.1) : (bet.dir === 'up' ? bet.entryPrice - 0.1 : bet.entryPrice + 0.1);
            } else {
        ...
        if (settled.length > 0) {
          ...
          // Знаходимо пару для popup
          let pair: Pair | undefined;
          if (first.id >= 1 && first.id <= 9) {
            pair = pairsRef.current.find((p) => p.id === first.id);
          } else if (first.id >= 101 && first.id <= 108) {
            pair = sportsAsPairs.find((p) => p.id === first.id);
          } else if (first.id >= 201 && first.id <= 208) {
            pair = newsAsPairs.find((p) => p.id === first.id);
          }
  ...
  const placeBet = (pairId: number, dir: Dir) => {
    ...
    // Знаходимо пару в залежності від категорії
    let pair: Pair | undefined;
    if (category === 'crypto') {
      pair = pairs.find((p) => p.id === pairId);
      if (!pair || !pair.price) return toast.error("No live price");
    } else if (category === 'sports') {
      pair = sportsAsPairs.find((p) => p.id === pairId);
      if (!pair) return toast.error("Event not found");
    } else if (category === 'news') {
      pair = newsAsPairs.find((p) => p.id === pairId);
      if (!pair) return toast.error("Event not found");
    }

    if (!pair) return toast.error("Pair not found");

    const now = Date.now();
    // Для крипто використовуємо live price, для інших - базову ціну
    const entry = category === 'crypto' ? pair.price : pair.price;
    const bet: Bet = {
      id: pairId,
      dir,
      amount,
    ...
    const actionLabel = 
      category === 'crypto' ? dir.toUpperCase() :
      category === 'sports' ? (dir === 'up' ? 'WIN' : 'LOSE') :
      (dir === 'up' ? 'BULLISH' : 'BEARISH');

    toast.success(
      `Bet placed: ${actionLabel} on ${pair.pair}`,
    );
    closeModal();
  };
          ...
          <div className="ui-card rounded-2xl max-h-[515px]  p-5 relative overflow-y-auto overflow-x-hidden">
            ...
            <div className="relative z-10 space-y-3">
              ...
              {activeBets.length === 0 ? (
                ...
                <div className="space-y-3">
                  {activeBets.map((bet) => {
                    // Знаходимо пару в залежності від ID
                    let pair: Pair | undefined;
                    if (bet.id >= 1 && bet.id <= 9) {
                      pair = pairs.find((pp) => pp.id === bet.id);
                    } else if (bet.id >= 101 && bet.id <= 108) {
                      pair = sportsAsPairs.find((pp) => pp.id === bet.id);
                    } else if (bet.id >= 201 && bet.id <= 208) {
                      pair = newsAsPairs.find((pp) => pp.id === bet.id);
                    }
                    ...
                    // Визначаємо label в залежності від категорії
                    let dirLabel = "LONG";
                    if (bet.id >= 1 && bet.id <= 9) {
                      dirLabel = bet.dir === "up" ? "LONG" : "SHORT";
                    } else if (bet.id >= 101 && bet.id <= 108) {
                      dirLabel = bet.dir === "up" ? "WIN" : "LOSE";
                    } else if (bet.id >= 201 && bet.id <= 208) {
                      dirLabel = bet.dir === "up" ? "BULLISH" : "BEARISH";
                    }
                    ...
                    const isCrypto = bet.id >= 1 && bet.id <= 9;
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2">
                              <div className="w-10 h-10 rounded-lg bg-[#eee]/5 flex items-center justify-center overflow-hidden">
                                ...
                                {pair && !isCrypto && bet.id >= 101 && bet.id <= 108 && (
                                  <Trophy className="w-6 h-6 text-orange-400" />
                                )}
                                {pair && !isCrypto && bet.id >= 201 && bet.id <= 208 && (
